...
public class _NetworkThread implements NetworkThread {
	...
	public _NetworkThread(_Server server, String port) {
		...
		ServerLogger.Log("NetworkManager succesfully initialized.",
	...
	public void run() {
		ServerLogger
				.Log("Opening ServerSocketChannel...", ServerLogger.MESSAGE);
		ServerSocketChannel serverChannel = null;
		try {
			serverChannel = ServerSocketChannel.open();
		} catch (IOException e1) {
			ServerLogger.Log("Failed to open ServerSocketChannel",
					ServerLogger.FATAL);
			System.exit(1);
		}
		ServerLogger.Log("Opening ServerSocketChannel was succesful!",
				ServerLogger.MESSAGE);
		try {
		...
		ServerLogger.Log("Setting blocking to false was succesful!",
				ServerLogger.MESSAGE);
		...
		ServerLogger.Log("Registration of Selector was succesful!",
				ServerLogger.MESSAGE);
		ServerLogger.Log("Binding to: " + "localhost:" + port,
				ServerLogger.MESSAGE);
		try {
			serverChannel.bind(new InetSocketAddress("localhost", Integer
					.parseInt(port)));
		} catch (NumberFormatException e1) {
			ServerLogger.Log("Failed to bind to address: " + "localhost",
					ServerLogger.FATAL);
			System.exit(1);
		} catch (IOException e1) {
			ServerLogger.Log("Failed to bind to address: " + "localhost",
	...
	private void loop() throws Throwable {
		...
		// ServerLogger.Log("(ServerThread): Current amount of selected keys: "
		// + selector.selectedKeys().size(), ServerLogger.DEBUG);
