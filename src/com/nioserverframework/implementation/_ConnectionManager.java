...
public class _ConnectionManager implements ConnectionManager {
	...
	public _ConnectionManager(_Server _server) {
		ServerLogger.Log("Initializing ConnectionManager...", ServerLogger.MESSAGE);
		this._server = _server;
		this.connections = Collections
				.newSetFromMap(new ConcurrentHashMap<Connection, Boolean>());
		ServerLogger.Log("ConnectionManager succesfully initialized.", ServerLogger.MESSAGE);
	}
	...
	public boolean disconnect(Connection c) {
		...
		ServerLogger.Log("(ConnectionManager): Connection from: " + c.getAddress() + " has been removed.", ServerLogger.DEBUG);
		return false;
	}
	...
	public Connection addConnection(SocketChannel s) {
		...
		ServerLogger.Log("(ConnectionManager): Connection from: " + c.getAddress() + " has been added.", ServerLogger.DEBUG);
		return c;
	}
