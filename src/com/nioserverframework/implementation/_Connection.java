		...
		try {
			if (readbuf.limit() == 4) {
				...
				if (!readbuf.hasRemaining()) {
					...
					connectionmanager.getServer().getNetworkThread().getInboundQueue().offer(p);
		...
		try {
			if (currentwritepacket != null) {
				if (writebuf.limit() == 4) {
					...
					if (!writebuf.hasRemaining()) {
						writebuf = ByteBuffer.wrap(currentwritepacket.getBytes());
						((_NetworkThread) ((_Server) server).getNetworkThread()).queueInterestChange(
